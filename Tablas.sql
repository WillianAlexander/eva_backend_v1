...
CREATE TABLE EVA.PARTICIPANTES (
	ID INTEGER GENERATED ALWAYS AS IDENTITY UNIQUE,
	NOMBRE VARCHAR(100) NOT NULL,
	APELLIDO VARCHAR(100) NOT NULL,
	CORREO VARCHAR(50) UNIQUE NOT NULL,
	DEPARTAMENTO_ID INTEGER NOT NULL REFERENCES EVA.DEPARTAMENTOS(id),
	FHASTA TIMESTAMP NOT NULL DEFAULT '2999-12-31 00:00:00',
    FDESDE TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	VERSION INTEGER NOT NULL DEFAULT 0
);

ALTER TABLE IF EXISTS EVA.PARTICIPANTES
    OWNER to postgres;
...
CREATE TABLE EVA.EVENTOPARTICIPANTES (
    ...
    PARTICIPANTE_ID INTEGER NOT NULL REFERENCES EVA.PARTICIPANTES(ID) ON DELETE CASCADE,
...
CREATE TABLE EVA.EVALUACIONES (
    ...
    EVENTO_ID INTEGER NOT NULL REFERENCES EVA.EVENTOS(ID),
    EVALUADOR_ID INTEGER NOT NULL REFERENCES EVA.PARTICIPANTES(ID),
    EVALUADO_ID INTEGER NOT NULL REFERENCES EVA.PARTICIPANTES(ID),
    RESPONSABILIDAD INTEGER NOT NULL CHECK (RESPONSABILIDAD BETWEEN 0 AND 10),
    COMUNICACION INTEGER NOT NULL CHECK (COMUNICACION BETWEEN 0 AND 10),
    GESTION INTEGER NOT NULL CHECK (GESTION BETWEEN 0 AND 10),
	COMENTARIO VARCHAR(1000) NOT NULL,
    CONSTRAINT sin_autoevaluacion CHECK (EVALUADOR_ID <> EVALUADO_ID),
    -- UNIQUE (FEVALUACION, EVENTO_ID, EVALUADOR_ID, EVALUADO_ID)
